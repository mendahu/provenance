...
| **Context** | Source doc §5.1; [`seeded-vocabulary.md`](../../seeded-vocabulary.md) §1.1 (`origin`). Simplest Spike 2 UI; no dependency on Sources catalog or type↔field suggestions. Recommend `provenencia` rows view-only for edits; `user` rows editable. |
...
| **Context** | Mount under S2-14 **Source fields** pane — do not reintroduce a separate top-level window chrome. Prefer `provenencia` rows read-only for mutations. |
